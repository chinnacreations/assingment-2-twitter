...
app.get(
  ...
  async (request, response) => {
    ...
    response.send({ likes: likedby.map((eachlike) => eachlike.username) });
  }
);
...
app.get(
  ...
  async (request, response) => {
    ...
    const replies = await db.all(
      `SELECT user.name, reply.reply FROM reply INNER JOIN user WHERE tweet_id = '${tweetId}';`
    );
...
app.get("/user/tweets/", authenticateToken, async (request, response) => {
  ...
  response.send(
    myTweets.map((eachItem) => {
      const { date_time, ...rest } = eachItem;
...
// Path: /tweets/:tweetId/
// Method: DELETE
// Description:If the user requests to delete a tweet of other users
app.delete(
  "/tweets/:tweetId/",
  authenticateToken,
  async (request, response) => {
